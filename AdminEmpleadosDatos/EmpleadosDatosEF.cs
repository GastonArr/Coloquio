...
namespace AdminEmpleadosDatos
{
    public static class EmpleadosDatosEF
    {
        ...
        {
            empleadosContext = new AdminEmpleadosDBContext();// creo una instancia usando el contructor sin parametros

            if (empleadosContext.empleado == null)
            {
                return new List<Empleado>(); //compruebo que la tabla empleado exista. Si no existe, se devuelve una lista vacía.
            }
            //Lazy Loading
            //List<Empleado> list = empleadosContext.empleado.ToList(); //sin departamentos

            List<Empleado> list;
            if (String.IsNullOrWhiteSpace(e.Nombre) && String.IsNullOrWhiteSpace(e.Dni))//si no se filtró por texto, se consulta toda la tabla 
            {
                list = empleadosContext.empleado.Include("Departamento")//incluyendo el Departamento.
                    //filtra segun el valor del parametro anulado enviado desde la capa de presentacion
                    .Where(emp => emp.anulado == e.anulado)//AG//compara el valor de cada fila con el recibido; solo pasan los registros que coincide.
                    .ToList();
            }
            else
            {
